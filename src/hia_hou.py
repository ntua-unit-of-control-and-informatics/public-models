...
lr = LogisticRegression(C=50, penalty='elasticnet', solver='saga', l1_ratio=0.5, random_state=42)
...
if args.run_as == 'single':
    ...
    # Train the model
    model = MolecularSKLearn(jaq_train, doa=Leverage(), model=lr, eval=val)
    _ = model.fit()
    ...
    model = MolecularSKLearn(dummy_train, doa=Leverage(), model=lr, eval=val)
